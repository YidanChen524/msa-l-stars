...
def simulate_random_sequences(n, m):
    ...
    with open(f"{os.path.dirname(os.path.abspath(__file__))}/experiments/random_{n}_{m}.fa", "w") as f:
        for i in range(n):
            f.write(f">seq{i}\n")
...
def simulate_related_sequences(n, m):
    """Simulate n sequences of length m."""
    with open(f"{os.path.dirname(os.path.abspath(__file__))}/experiments/related_{n}_{m}.fa", "w") as f:
...
def convert_alignment_to_sequences(file, n, m):
    ...
    with open(f"{os.path.dirname(os.path.abspath(__file__))}/experiments/simulated_{n}_{m}.fa", "w") as f:
